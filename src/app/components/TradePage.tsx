...
const tradingPairs = [
  { id: 1, symbol: 'HAZEL', name: 'Hazelnut Farm', price: 125.50 },
  { id: 2, symbol: 'CORN', name: 'Organic Corn Farm', price: 89.30 },
  { id: 3, symbol: 'RICE', name: 'Rice Paddy Valley', price: 205.80 },
  { id: 4, symbol: 'SOY', name: 'Soybean Collective', price: 156.20 },
];
...
export function TradePage({ selectedTokenSymbol, onNavigateToAbout }: TradePageProps) {
  ...
  const initialToken = selectedTokenSymbol 
  ...
  const [price, setPrice] = useState(initialToken.price.toString());
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const { theme } = useTheme();
  
  const availableBalance = 500; // MKOIN
  const maxAmount = Math.floor(availableBalance / selectedPair.price); // Max coins user can buy
