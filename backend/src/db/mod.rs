...
impl Database {
    ...
    pub async fn create_user(&self, address: &str, role: &str, name: Option<&str>) -> Result<Uuid> {
    ...
    pub async fn get_user_portfolio(
        &self,
        user_address: &str,
    ) -> Result<Vec<crate::api::PortfolioItem>> {
    ...
    pub async fn upsert_token_minter(
        &self,
        address: &str,
        symbol: Option<&str>,
        ...
        sqlx::query!(
            r#"
            INSERT INTO token_minters (address, symbol, metadata_url, is_agri_token, total_supply, campaign_id, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6, NOW())
            ON CONFLICT (address)
            DO UPDATE SET
                symbol = COALESCE(EXCLUDED.symbol, token_minters.symbol),
                metadata_url = COALESCE(EXCLUDED.metadata_url, token_minters.metadata_url),
                is_agri_token = EXCLUDED.is_agri_token,
                total_supply = COALESCE(EXCLUDED.total_supply, token_minters.total_supply),
                campaign_id = COALESCE(EXCLUDED.campaign_id, token_minters.campaign_id),
                updated_at = NOW()
            "#,
            address,
            symbol,
    ...
    pub async fn get_token_minter(&self, address: &str) -> Result<Option<Token>> {
        let token = sqlx::query_as!(
            Token,
            r#"
            SELECT address, symbol, metadata_url, is_agri_token, total_supply, campaign_id, created_at, updated_at
            FROM token_minters
            WHERE address = $1
            "#,
            address
